...
/**
 * Validate a DHCP range against a CIDR + gateway.
 * - both ends must be usable hosts in the CIDR
 * - start <= end
 * - the gateway must NOT fall inside the range (otherwise dnsmasq would
 *   try to lease the gateway IP and PVE rejects the subnet)
 */
export interface DhcpRangeValidation {
  ok: boolean
  /** Why it failed, untranslated tag — UI maps to i18n. */
  reason?: 'invalid_start' | 'invalid_end' | 'reversed' | 'gateway_in_range'
}
...
export function validateDhcpRange(
  ...
  if (!ipInCidrUsable(start, cidr)) return { ok: false, reason: 'invalid_start' }
  if (!ipInCidrUsable(end, cidr)) return { ok: false, reason: 'invalid_end' }
  const startInt = ipToInt(start)!
  const endInt = ipToInt(end)!
  if (startInt > endInt) return { ok: false, reason: 'reversed' }
  const gwInt = ipToInt(gateway)
  if (gwInt !== null && gwInt >= startInt && gwInt <= endInt) {
    return { ok: false, reason: 'gateway_in_range' }
  }
  return { ok: true }
}

/** First usable IP after the gateway — useful as a default DHCP range start. */
export function firstUsableAfterGateway(cidr: string, gateway: string): string | null {
  const p = parseCidr(cidr)
  const gwInt = ipToInt(gateway)
  if (!p || gwInt === null) return null
  // If gateway is the firstUsable, start from gw+1; else just start from firstUsable.
  const candidate = gwInt === p.firstUsableInt ? gwInt + 1 : p.firstUsableInt
  if (candidate > p.lastUsableInt) return null
  return intToIp(candidate)
}

/** Last usable IP — handy as a default DHCP range end. */
export function lastUsableIp(cidr: string): string | null {
  const p = parseCidr(cidr)
  if (!p) return null
  return intToIp(p.lastUsableInt)
}
