...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    try {
      const pools = await pbsFetch<any[]>(conn, "/nodes/localhost/disks/zfs")

      return NextResponse.json({ data: Array.isArray(pools) ? pools : [] })
    } catch (inner: any) {
    ...
    console.error("PBS disks/zfs GET error:", e)
