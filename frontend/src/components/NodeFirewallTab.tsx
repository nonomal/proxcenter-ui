...
export default function NodeFirewallTab({ connectionId, node }: Props) {
  ...
  const api = useMemo<FirewallAPIAdapter>(() => {
    const base = `/api/v1/firewall/nodes/${connectionId}/${node}`
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 6, sm: 3 }}>
                <Paper sx={{ p: 2, bgcolor: alpha(theme.palette.background.default, 0.5), textAlign: 'center' }}>
                  ...
                  <Chip
                    size="small"
                    label={fw.options.nf_conntrack_max ? String(fw.options.nf_conntrack_max) : 'Default'}
                    sx={{ height: 26 }}
                  />
                </Paper>
              </Grid>
              <Grid size={{ xs: 6, sm: 3 }}>
                <Paper sx={{ p: 2, bgcolor: alpha(theme.palette.background.default, 0.5), textAlign: 'center' }}>
                  ...
                  <Chip
                    size="small"
                    label={fw.options.tcpflags === 1 ? t('common.enabled') : t('common.disabled')}
                    color={fw.options.tcpflags === 1 ? 'success' : 'default'}
                    sx={{ height: 26 }}
                  />
                </Paper>
              </Grid>
